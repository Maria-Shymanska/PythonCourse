...
class Player(AbstractPlayer):
    ...
    def render(self, screen: Screen):
        ...
        if self.kaijus_killed < self.kaijus_killed_high:
            self.screen.status[self.score_title] += " ({} K)".format(
                self.kaijus_killed_high
            )
        ...
        if self.active:
            ...
            if self.flame_on:
                ...
                self.flamethrower.color = choice(
                    [screen.COLOR_RED, screen.COLOR_YELLOW]
                )
    ...
    def destruct(self):
        ...
        self.screen.add(
            Stickman(
                self.shape.x, self.shape.y, color=self.screen.COLOR_YELLOW, y_delta=-0.1
            )
        )
...
class Enemies(AbstractEnemies, KeyListener):
    def create_enemy(self):
        ...
        return Wasp(
            x,
            y,
            x_delta=x_sign * speed,
            y_delta=y_sign * speed,
            random_start=True,
            flip=x_sign > 0,
    ...
    def on_death(self, enemy):
        enemy_class = (
            DyingWaspKaiju if isinstance(enemy, CompassionateBoss) else enemy.__class__
        )
        wasp = enemy_class(
            enemy.x,
            enemy.y,
    ...
    def additional_enemies(self):
        return super().additional_enemies() + abs(
            (self.player.obstacles.x - self.player.x) / 50
        )
