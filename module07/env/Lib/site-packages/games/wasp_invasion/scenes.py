...
class Intro(Scene):
    def init(self):
        self.intro = Monologue(
            self.controller.player.x,
            self.controller.player.y - 3,
            on_finish=self.next,
            texts=[
                "Hi, I am {}.".format(self.controller.player.name),
...
class Survive(Scene):
    def init(self):
        self.enemies = Enemies(
            self.controller.player, max_enemies=int(self.screen.width / 10)
        )
        self.landscapes = (
        ...
        self.obstacles = Obstacles(
            self.screen.width / 2, self.screen.height - 6, player=self.player
        )
        self.player.reset()
