...
class ScreenObject:
    ...
    def __init__(
        ...
        self._random_movement = random_movement or getattr(
    ...
    def is_out(self):
        ...
        try:
            return (
                self.x + self.size < 0
                or self.x - self.size > self.screen.width
                or self.y + self.size < 0
                or self.y - self.size > self.screen.height
            )
    ...
    def shifted_coords(self, x_delta=0, y_delta=0):
        ...
        y_adjustment = (
            (self.y - int(self.y)) * 1.1 if y_delta else 0
        )  # Times 1.1 to avoid stuck in rock
        new_coords = set()
        for x, y in self.coords:
            new_coords.add(
                (int(x + x_delta + x_adjustment), int(y + y_delta + y_adjustment))
            )
        return new_coords
...
class AbstractPlayer(ScreenObject, KeyListener):
    ...
    def can_move_x(self, x_delta=0):
        ...
        return (
            self.screen
            and (
                self.x + x_delta > self.size
                and self.x + x_delta < self.screen.width - self.size
            )
            and (
                not self.obstacles
    ...
    def can_move_y(self, y_delta=0):
        ...
        return (
            self.screen
            and (
                self.y + y_delta > self.size / 2
                and self.y + y_delta < self.screen.height - self.size / 2
            )
            and (
                not self.obstacles
    ...
    def destruct(self, msg=None, explode=True, explosion_size=30, on_finish=None):
        ...
        if msg:
            self.screen.add(
                Text(self.screen.width / 2, self.screen.height / 2, msg, centered=True)
            )
        if explode:
            self.screen.add(
                Explosion(
...
class CompassionateBoss(ScreenObject):
    def __init__(self, name, shape: ScreenObject, player: AbstractPlayer, hp=5):
        super().__init__(
            ...
            size=shape.size,
            color=shape.color,
            x_delta=shape.x_delta,
            y_delta=shape.y_delta,
            player=player,
        )
        self.name = name
        self.shape = shape
    ...
    def render(self, screen: Screen):
        ...
        if self in self.screen:
            ...
            if self.is_hit:
                self.color = self.screen.rainbow_colors[
                    self.screen.renders % len(self.screen.rainbow_colors)
                ]
...
class Bitmap(ScreenObject):
    def __init__(
        ...
        **kwargs
        ...
        self._remove_after_animation = getattr(
            self, "remove_after_animation", remove_after_animation
        )
        self._bitmap_index_offset = (
            randint(0, max(len(self._bitmaps), 1) - 1) if random_start else 0
        )
        self.flip = flip
...
""",
    ...
    def draw(self, x, y, char, screen: Screen):
        color = (
            self.color[self.renders % len(self.color)]
            if type(self.color) in (tuple, list)
            else self.color
        )
    ...
    def render(self, screen: Screen):
        ...
        if self._bitmaps:
            index = (
                self._bitmap_index_offset + int(self.renders / self._frames_per_bitmap)
            ) % len(self._bitmaps)
        ...
        for y in range(start_y, start_y + self.size):
            ...
            for x in range(start_x, start_x + x_size):
                ...
                if (
                    x_offset < len(bitmap[y_offset])
                    and bitmap[y_offset][x_offset] != " "
                ):
        ...
        if self._remove_after_animation and self.renders > self._frames_per_bitmap * (
            len(self._bitmaps) - 1
        ):
            screen.remove(self)
            if self.parent:
...
class ObjectMap(Bitmap):
    ...
    def draw(self, x, y, char, screen):
        ...
        if char in self._object_map:
            if (
                x > screen.width + self.grid_size
                or y > screen.height + self.grid_size
                or x < -self.grid_size
                or y < -self.grid_size
            ):
                return
...
class Text(ScreenObject):
    def __init__(
        ...
        super().__init__(
            x, y, size=int(len(text) / 2), x_delta=x_delta, y_delta=y_delta, color=color
        )
        self.text = text
        self.centered = centered
...
class Monologue(Text):
    def __init__(
        self, x: int, y: int, texts=[], on_finish=None, x_delta=None, y_delta=None
    ):
    ...
    def render(self, screen: Screen):
            ...
            for y in range(screen.height):
                if x == 0 or y == 0 or x == screen.width - 1 or y == screen.height - 1:
                    char = (
                        ...
                        or (x == 0 and y == 0)
                        and chr(0x2554)
                        or (x == screen.width - 1 and y == 0)
                        and chr(0x2557)
                        or (x == 0 and y == screen.height - 1)
                        and chr(0x255A)
                        or (x == screen.width - 1 and y == screen.height - 1)
                        and chr(0x255D)
                        or (y == 0 or y == screen.height - 1)
                        and chr(0x2550)
                        or (x == 0 or x == screen.width - 1)
                        and chr(0x2551)
                    )
                    ...
                    if (
                        self.energy_level
                        and x == screen.width - 1
                        and y < screen.height - 1
                    ):
                        y_level = int((screen.height - 2) * self.energy_level + 0.5)
                        if screen.height - 1 - y <= y_level:
                            screen.draw(
                                screen.width - 2, y, "│", color=screen.COLOR_GREEN
                            )
        ...
        if self.title:
            ...
            for x_offset in range(len(padded_title)):
                screen.draw(
                    start_x + x_offset, 0, padded_title[x_offset], color=self.color
                )
...
class Circle(ScreenObject):
    def __init__(
        ...
        super().__init__(x, y, x_delta=x_delta, y_delta=y_delta, color=color, size=size)
        self.char = char
        self.name = name
    ...
    def render(self, screen: Screen):
        ...
        for x in range(start_x, int(start_x + self.size)):
            for y in range(start_y, int(start_y + self.size)):
                if self.solid or (
                    x == start_x
                    or x == start_x + self.size - 1
                    or y == start_y
                    or y == start_y + self.size - 1
                ):
    ...
    def render(self, screen: Screen):
        ...
        for x in range(start_x, start_x + self.current_size):
            for y in range(start_y, start_y + self.current_size):
                if (
                    x == start_x
                    or x == start_x + self.current_size - 1
                    or y == start_y
                    or y == start_y + self.current_size - 1
                ):
                    distance = ((x - self.x) ** 2 + (y - self.y) ** 2) ** (1 / 2)
                    if (
                        distance < self.current_size
                        and random() < 2 / self.current_size
                    ):
                        self.coords.add((int(x), int(y)))
                        screen.draw(
                            x, y, self.char, color=colors[randint(0, len(colors) - 1)]
                        )
...
class Triangle(ScreenObject):
    ...
    def render(self, screen: Screen):
        ...
        if self.size >= 2:
            self.coords.update(
                {(int(self.x - 1), int(self.y)), (int(self.x + 1), int(self.y))}
            )
            if self.size == 2:
...
class Choice(ScreenObject, KeyListener):
    def __init__(
        ...
        self._current = (
            current if current is not None else int(len(self.choices) / 2 - 0.5)
        )
    ...
    def render(self, screen: Screen):
        ...
        if not self.bar:
            ...
            self.bar = Bar(
                ...
                self.y + max_size / 2 + 1,
                size=bar_size,
                color=self.color or screen.COLOR_CYAN,
            )
            screen.add(self.bar)
...
class Landscape(ObjectMap, KeyListener):
    move_speed = 0
    object_map = {"S": Sun, "T": Tree, "R": Rock, "V": Volcano}
