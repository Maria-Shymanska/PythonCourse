            ...
            if self.gas > 0:
                ...
                screen.add(
                    Monologue(
                        self.x,
                        self.y - 2,
                        on_finish=self.destruct,
                        texts=["Out of gas!!!", "Better fly faster next time!"],
                    )
                )
                self.gas = -1
                self.y_delta = 0.1
...
class SpinnerEnemies(AbstractEnemies, KeyListener):
    def create_enemy(self):
        ...
        return Spinner(
            x, y, y_delta=y_delta, player=self.player, explode_on_impact=True
        )
...
class XEnemies(AbstractEnemies, KeyListener):
    def create_enemy(self):
        ...
        thetas = [0.75, -0.75]
        x_points = set(
            ((i + 1) * self.size * 2, thetas[i % 2]) for i in range(self.max_enemies)
        )
        ...
        return X(
            ...
            y_delta=y_delta,
            player=self.player,
            magnify_by_size=True,
            size=self.size,
            rotate_axes=(0, 0, 1, theta_factor),
        )
