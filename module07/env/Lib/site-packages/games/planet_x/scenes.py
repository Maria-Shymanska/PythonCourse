...
from games.planet_x.objects import (
    Landscape1,
    Landscape2,
    Obstacles,
    CrabClawEnemies,
...
class Intro(Scene):
    def init(self):
        ...
        self.intro = Monologue(
            self.controller.player.x - 1,
            self.controller.player.y + 4,
            on_finish=self.next,
            texts=[
        ...
        self.landscapes = (
            Sun(self.screen.width / 2, 2),
            Landscape1(0, self.screen.height / 2 - 5, player=self.player),
            Landscape2(0, self.screen.height / 2, player=self.player),
        )
        self.obstacles = Obstacles(
            -self.screen.width, self.screen.height - 6, player=self.player
        )

    def start(self):
        self.screen.add(
            *self.landscapes, self.obstacles, self.intro, self.controller.player
        )
...
class WormholeSucks(WormholeAppeared):
    def init(self):
        ...
        self.intro = Monologue(
            self.controller.player.x - 1,
            self.controller.player.y + 4,
            texts=["Oh no!!", "We are being sucked in!!", "Hold on ti..."],
        )
...
class Level1(Scene):
    def init(self):
        self.level = One(
            self.screen.width / 2, self.screen.height / 2, remove_after_renders=30
        )
        self.player.reset()
        self.player.score = 1
        self.enemies = CrabClawEnemies(
            self.controller.player, max_enemies=int(self.screen.width / 20)
        )
        self.wormhole = Wormhole(6, self.screen.height / 2)
        self.wormhole.player = self.controller.player
        self.wormhole.scene = self
        self.intro = Monologue(
            self.controller.player.x - 2,
            self.controller.player.y + 4,
            texts=[
                "Looks like...",
    ...
    def start(self):
        self.screen.add(
            self.level, self.enemies, self.player, self.wormhole, self.intro
        )
...
class Level2(Scene):
    def init(self):
        ...
        self.enemies = AcidBubbleEnemies(
            self.controller.player, max_enemies=int(self.screen.width / 17.5)
        )
        self.wormhole = Wormhole(6, self.screen.height / 2)
        self.wormhole.player = self.controller.player
        self.wormhole.scene = self
...
class Level3(Scene):
    def init(self):
        ...
        self.enemies = VolcanoEnemies(
            self.controller.player, max_enemies=int(self.screen.width / 15)
        )
        self.wormhole = Wormhole(6, self.screen.height / 2)
        self.wormhole.player = self.controller.player
        self.wormhole.scene = self
...
class Level4(Scene):
    def init(self):
        ...
        self.enemies = JellyFishEnemies(
            self.controller.player, max_enemies=int(self.screen.width / 12.5)
        )
        self.wormhole = Wormhole(6, self.screen.height / 2)
        self.wormhole.player = self.controller.player
        self.wormhole.scene = self
...
class Level7(Scene):
    def init(self):
        ...
        self.enemies = XEnemies(
            self.controller.player, max_enemies=max_enemies, size=size
        )
        self.wormhole = Wormhole(6, self.screen.height / 2)
        self.wormhole.player = self.controller.player
        self.wormhole.scene = self
...
class Home(Scene):
    def init(self):
        ...
        self.outro = Monologue(
            self.controller.player.x - 1,
            self.controller.player.y + 4,
            texts=[
                "Yay!! We are finally home!",
    ...
    def start(self):
        self.screen.add(
            *self.landscapes, self.obstacles, self.outro, self.controller.player
        )
