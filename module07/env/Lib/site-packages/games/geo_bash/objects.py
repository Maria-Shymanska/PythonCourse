...
class Player(AbstractPlayer):
    ...
    def render(self, screen: Screen):
        ...
        if (self.screen.renders % 2 == 0 or self.char == "!") and self.active:
            if self.char == "!":
                color = self.screen.rainbow_colors[
                    int(self.screen.renders / 2) % len(self.screen.rainbow_colors)
                ]
            else:
                color = self.color
            ...
            if self.char == "O":
                ...
                projectile = Projectile(
                    self.x, self.y - 1, shape=self.char, parent=self, color=color
                )
                self.kids.add(projectile)
                self.screen.add(projectile)
