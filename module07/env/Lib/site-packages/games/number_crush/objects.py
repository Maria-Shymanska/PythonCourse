...
class Player(AbstractPlayer):
    def __init__(self, controller):
        super().__init__(
            "Jon",
            Stickman(int(controller.screen.width / 2), controller.screen.height - 3),
            controller,
            score_title="Crushed",
            show_total=True,
        )
...
class Numbers(ScreenObject, KeyListener):
    ...
    def number_pressed(self, number):
        if self.kids and self.player.alive:
            ...
            player_answer = eval(
                "".join(
                    [str(n) for n in self.numbers_pressed[-len(str(answer)) :]]
                ).lstrip("0")
                or "0"
            )

            if self.last_answer != player_answer and len(str(player_answer)) == len(
                str(answer)
            ):
                answer = Text(
                    self.player.x,
